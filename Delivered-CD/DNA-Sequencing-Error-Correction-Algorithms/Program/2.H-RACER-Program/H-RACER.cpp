			...
			if(hasCorr){
				...
				while (lasIdx <= i) 
				{
					...
					if(charReads[lasIdx] != crrVals[crrValIdx]){
						...
						} else if(crrReads[lasIdx + crrIdx - 1] != '\n' && crrReads[lasIdx + crrIdx - 1] != '\r' //current is not the first nuc of read
							      &&
						...
						} else{ //The first/last nucleotide OR the next/prev is not error
		                    //Substitute
							crrReads[lasIdx + crrIdx] = crrVals[crrValIdx];	
						}
					}else{
...
int main(int argc, char* argv[]) 
{
	...
	if (argc != 4) {
		...
		cout << "Usage: ./hracer <inputReads> <correctedReads> <genomeLength>" << endl << flush;
