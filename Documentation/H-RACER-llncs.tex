...
\\ 
The number of reads that include a given nucleotide in the sequence is called ``Coverage". The coverage rate of a genome sequences is calculated by the following equation \ref{eq:coverage}
...
\newpage
\chapter{\label{chap:4}KGEC Algorithm}
This chapter has two main sections, methodology and evaluation. The methodology section explains the working methodology of KGEC (the first proposed algorithm), while the evaluation section gives a briefed discussion about the results of the comparisons established between KGEC and the other existent algorithms, where the detailed discussion of the results will be shown in chapter \ref{chap:6}.
\\
\\
...
The testing was performed on a wide variety of real datasets, shown below in table \ref{tab:eval-data}, with different read length, genome size and coverage. It was preferred to use real datasets and to avoid any simulated ones as they do not offer a good indication of real life performance. 
\\
...
\\
More details about the used datasets will be explained in subsection \ref{sec:eval-data}.

\subsection{Results and Discussion}
The comparisons, shown below in table \ref{tab:eval-0}, were established between H-RACER and algorithms specialized in correcting all types of errors (substitutions, insertions, and deletions). While the obtained measurements were brought via the verification code implemented by RACER, that has the advantage of avoiding the interference of mapping or assembling programs. 
\\\\
...
\\
\\
While for Treponema Pallidum, E.coli 75a and E.coli 75b, data with larger size, the algorithm has been running for too many hours while other algorithms shows their results in minutes; consequently it enforces the interruption of the running without getting the results.
\\
\\
The defect in time of this algorithm caused from the k-mers grouping steps illustrated above in section \ref{sec:alg1-meth}, as the complexity of the k-mers grouping step in exponential of the k-mers count. More detailed explanation will be shown in subsection \ref{subsec:eval1-res}.
  
\newpage
\chapter{\label{chap:5}H-RACER Algorithm}
This chapter has two main sections, methodology and evaluation. The methodology section explains the working methodology of H-RACER (the second proposed algorithm), while the evaluation section gives a briefed discussion about the results of the comparisons established between H-RACER and the other existent algorithms, where the detailed discussion of the results will be shown in chapter \ref{chap:6}.
\\
\\
...
More details about the used datasets will be explained in subsection \ref{sec:eval-data}.
\subsection{Results}
The comparisons, shown in tables \ref{tab:eval-1}, \ref{tab:eval-2}, \ref{tab:eval-3}, and \ref{tab:eval-4}, were established between H-RACER and algorithms specialized in correcting all types of errors (substitutions, insertions, and deletions). While the obtained measurements were brought via the verification code implemented by RACER, that has the advantage of avoiding the interference of mapping or assembling programs. 
\\\\
As shown in the comparisons tables, H-RACER has the best results in accuracy and time. Actually, H-RACER aims to increase the genome reads accuracy, consequently, it aims to eliminate the errors existing in the genome reads. So, H-RACER avoided introducing errors to the reads by lowering the false positive rate and consequently increasing the specificity rate. On the other side, the false negative rate was negatively affected and the sensitivity rate was decreased as well. But, this approach did not negatively affect the accuracy, on contradictory, it resulted in getting the best accuracy.
\\\\
The comparisons with the other algorithms, shown below in tables \ref{tab:eval-1}, \ref{tab:eval-2}, \ref{tab:eval-3}, and \ref{tab:eval-4}, prove that although the other algorithms have higher sensitivity than H-RACER, but all of them do not explicitly beat H-RACER accuracy, except for \enquote{Treponema Pallidum}, where HSHREC beats H-RACER's accuracy by 0.07\% and this is due to the high coverage rate of \enquote{Treponema Pallidum} that will be discussed in details in subsection \label{subsec:eval2-res}.  
...
As stated before, the testing of both proposed algorithms was performed on a wide variety of real datasets, shown below in table \ref{tab:eval-data}, with different read length, genome size and coverage. It was preferred to use real datasets and to avoid any simulated ones as they do not offer a good indication of real life performance. 
\\
...
\section{Results}
\subsection{\label{subsec:eval1-res}KGEC Algorithm}
As stated before, the comparisons, shown below in table \ref{tab:eval-0}, were established between H-RACER and algorithms specialized in correcting all types of errors (substitutions, insertions, and deletions). While the obtained measurements were brought via the verification code implemented by RACER, that has the advantage of avoiding the interference of mapping or assembling programs. 
\\\\
For Lactococcus Lactis, the data with smallest size, the proposed algorithm shows the best accuracy with a very small difference (0.05\%), but with the worst time with a big difference; 1 hr compared to 15, 5, and 3 mins.
\\
\\
...
As stated before, the comparisons, shown below in tables \ref{tab:eval-1}, \ref{tab:eval-2}, \ref{tab:eval-3}, and \ref{tab:eval-4}, were established between H-RACER and algorithms specialized in correcting all types of errors (substitutions, insertions, and deletions). While the obtained measurements were brought via the verification code implemented by RACER, that has the advantage of avoiding the interference of mapping or assembling programs. 
\\\\
...
\\
\\
KGEC (the first algorithm) available at: 
\\
...
\\
\\
And finally, KGEC (the first proposed algorithm) can get another chance by inventing a new algorithm to group the k-mers in with a reasonable complexity instead of the exponential one.
