...
\\
\\
This thesis documentation is organized as follows; illustrating some basics about deoxyribonucleic acid (DNA), deoxyribonucleic acid (DNA) sequencing, next-generation sequencing (NGS), and next-generation sequencing (NGS) errors correction in Chap. \ref{chap:1}, while the  existing methodologies for error correction are demonstrated in Chap. \ref{chap:2}, then the first proposed algorithm is demonstrated and evaluated in Chap. \ref{chap:3}, while the second proposed algorithm is demonstrated in Chap. \ref{chap:4}. In Chap. \ref{chap:5}, a comparison is set between the new methodology of the second proposed algorithm versus the existent ones through experimental data. The conclusion and future work are shown in Chap. \ref{chap:6}.
%
...
This proposal aims to correct all types of errors
...
\begin{figure}
	...
	\label{fig:fig13}
	\caption[caption]{First Proposal Steps}
\end{figure}
...
\newpage
\section{\label{sec:3}The Second Proposed Algorithm}
...
\newpage
\section{\label{sec:5}Conclusion and Future Work}
...
\\
\\
As a future work, the main points will be for enhancing the memory usage of H-RACER for long genomes, so as to be able to run long genomes within 244GiB RAM.
\\
Also, implementing H-RACER with parallel threads, where both time and memory will be enhanced, especially for long genomes.
\\
And finally, get another trial in inventing a new algorithm to group the k-mers in the first algorithm with a reasonable complexity.
\\
Finally, H-RACER algorithm has been implemented in C/C++ as an open source program.
\\
\\
Available at: https://github.com/salmagomaa/H-RACER \\
